#+TITLE: xtr — Fast Semantic Data Extractor for Linux
#+AUTHOR:
#+OPTIONS: toc:nil num:nil

~xtr~ is a standalone, high-performance command-line utility built in Go that functions like ~grep~, but is specialized for *structured data types and entity extraction*. It combines regular expressions, algorithmic checksum validators (Luhn, Mod-97, Mod-11), and a natural language query router.

Unlike standard ~grep~, which prints entire lines by default and requires arcane regular expressions, ~xtr~ defaults to extracting *only the matched pattern* (like ~grep -o~), supports natural language subcommands (~xtr "find all emails"~), and applies mathematical validation to the types that carry check digits.
...
- *Natural Language Intent Recognition*: Run ~./xtr urls~, ~./xtr "find emails"~, or ~./xtr "show me credit cards"~ without remembering strict syntax.
...
- *UNIX Pipeline Streaming*: Reads from files or standard input (~cat log | xtr -u ips~) with bounded memory usage using a sliding-window ring buffer.
...
~xtr~ is written in standard Go with zero third-party dependencies. There is no ~go.mod~ — the single source file is compiled directly.

** Build as a Static ELF Binary

#+begin_src bash
# Build standalone binary stripped of debug symbols
CGO_ENABLED=0 go build -ldflags="-s -w" -o xtr xtr.go

# Verify it has zero dynamic dependencies
file xtr
# Output: xtr: ELF 64-bit LSB executable, ..., statically linked, stripped

# Move to a system or user PATH directory
chmod +x xtr
mv xtr /usr/local/bin/   # or ~/bin/
#+end_src

** Run Directly Without Compiling

#+begin_src bash
go run xtr.go urls access.log
#+end_src
...
#+begin_example
Usage:
  xtr [options] <data-type | "natural language query"> [file ...]
  cat input.txt | xtr [options] <data-type | "query">
#+end_example
...
- ~-c~ reports the total number of matches, and is *not* reduced by ~-u~. ~xtr -c -u emails f~ prints the raw match count, not the number of distinct addresses. To count distinct values, use ~xtr -u emails f | wc -l~.
...
*This differs from ~grep~*, which exits ~1~ when nothing matches. A shell test like ~xtr ssns data.csv && alert~ will fire unconditionally. Test for output instead:

#+begin_src bash
[ -n "$(xtr ssns data.csv)" ] && alert
#+end_src
...
~xtr~ implements precise context controls where the count *includes* the matching line:
...
|------------------------------------------+--------------------|
| ~xtr urls~                               | ~urls~             |
| ~xtr "extract all web links"~            | ~urls~             |
| ~xtr "find email addresses"~             | ~emails~           |
| ~xtr "show me credit cards"~             | ~credit_cards~     |
| ~xtr "get ip addresses"~                 | ~ips~              |
| ~xtr "look for phone numbers"~           | ~phone_numbers~    |
| ~xtr "where are the mac addresses"~      | ~mac_addresses~    |
| ~xtr "list all bank accounts"~           | ~ibans~            |
| ~xtr "cve security bugs"~                | ~cves~             |
| ~xtr "find bitcoin or ethereum wallets"~ | ~crypto~           |
| ~xtr emaisl~ (typo)                      | ~emails~           |
| ~xtr phne~ (typo)                        | ~phone_numbers~    |

-----

* Supported Data Types & Categories

29 extractors across 16 categories. Run ~xtr list 2>&1~ to view them from the binary itself — the ~Registry~ in ~xtr.go~ is the authoritative source.
...
For the types that carry a check digit, ~xtr~ does not simply match digit lengths — it verifies the arithmetic. This is the "broad regex, then validate" pattern, and it is what keeps false positives low on the financial and identity types.
...
xtr --line "find credit cards" backup.sql

# Print lines mentioning CVE vulnerabilities:
xtr --line cves security_report.txt
#+end_src
...
- *~crypto~ does not extract Solana addresses*, despite the ~solana~ alias resolving and the description mentioning it. A ~reSolana~ pattern is compiled in ~xtr.go~ but is not wired into the ~crypto~ extractor. Wiring it in needs a false-positive story first: it is length-bounded Base58 with no checksum, so it would match a great many non-addresses.
...
- *Single File, Zero Dependencies*: All logic lives in ~xtr.go~ — validators, precompiled regexes, the ~Registry~, the intent router, and the stream processor. Adding a data type means adding a regex (plus a validator if it has a checksum) and appending one ~Extractor~ entry; flag parsing, help text, and the router all iterate the registry generically.
